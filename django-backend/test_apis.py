...
_created_item_ids = []
_created_audio_item_id = None
_created_test_id = None
...
try:
    ...
    if student:
        if not test_for_ai:
            _test_name = f"API Test {uuid.uuid4().hex[:6]}"
            test_for_ai = Test.objects.create(
                name=_test_name,
                type='writing',
                duration_minutes=30,
                total_questions=1,
                instructions='Auto-created for API tests',
                status='published',
            )
            _created_test_id = test_for_ai.id
    ...
    # Prefer component/item that actually belongs to the student's assigned test
    ASSIGN_CID = None
    ASSIGN_ITEM_ID = None
    ASSIGN_AUDIO_ITEM_ID = None
    if assignment and assignment.clap_test_id:
        _assign_component = ClapTestComponent.objects.filter(
            clap_test_id=assignment.clap_test_id
        ).first()
        if _assign_component:
            ASSIGN_CID = str(_assign_component.id)
            if hasattr(_assign_component, "items"):
                _assign_item = _assign_component.items.first()
                if _assign_item:
                    ASSIGN_ITEM_ID = str(_assign_item.id)

    # Ensure at least one item exists for the assigned component (deterministic tests)
    if ASSIGN_CID and not ASSIGN_ITEM_ID:
        _max_order = (
            ClapTestItem.objects.filter(component_id=ASSIGN_CID)
            .aggregate(Max("order_index"))
            .get("order_index__max") or 0
        )
        _text_item = ClapTestItem.objects.create(
            component_id=ASSIGN_CID,
            item_type="text_block",
            order_index=_max_order + 1,
            points=0,
            content={"text": "Test item"},
        )
        _created_item_ids.append(_text_item.id)
        ASSIGN_ITEM_ID = str(_text_item.id)

    # Ensure an audio_block item exists for audio-specific tests
    if ASSIGN_CID:
        _audio_item = ClapTestItem.objects.filter(
            component_id=ASSIGN_CID, item_type="audio_block"
        ).first()
        if not _audio_item:
            _max_order = (
                ClapTestItem.objects.filter(component_id=ASSIGN_CID)
            ...
            _audio_item = ClapTestItem.objects.create(
                component_id=ASSIGN_CID,
                item_type="audio_block",
                order_index=_max_order + 1,
                points=0,
                content={"play_limit": 2, "has_audio_file": False},
            )
            _created_item_ids.append(_audio_item.id)
            _created_audio_item_id = _audio_item.id
        ASSIGN_AUDIO_ITEM_ID = str(_audio_item.id) if _audio_item else None

    # If no general item was found for admin tests, fall back to assignment item
    if not ITEM_ID and ASSIGN_ITEM_ID:
    ...
    if ITEM_ID:
        ...
        record("CI3", "GET /admin/clap-items/{id}  (405 by design -- no GET detail)", resp, 405)
    ...
    if ASSIGN_ID and ASSIGN_CID and SID:
        resp = student_test_items(
            _req(
                "GET",
                f"/api/student/clap-assignments/{ASSIGN_ID}/components/{ASSIGN_CID}/items",
                None,
                student_id=SID,
            ),
            ASSIGN_ID,
        ...
        record(
            "SP5",
            "GET /student/clap-assignments/{asgn_id}/components/{comp_id}/items",
            resp,
            [200, 403],
        )
    else:
    ...
    if ASSIGN_ID and SID:
        if ASSIGN_AUDIO_ITEM_ID or ASSIGN_ITEM_ID:
            _item_for_audio = ASSIGN_AUDIO_ITEM_ID or ASSIGN_ITEM_ID
            _mime_list = getattr(settings, "AUDIO_ALLOWED_MIMETYPES", None) or []
            _ext_list = getattr(settings, "AUDIO_ALLOWED_EXTENSIONS", None) or []
            _mime = _mime_list[0] if _mime_list else "audio/mpeg"
            _ext = _ext_list[0] if _ext_list else "mp3"
            _size = min(1024, int(getattr(settings, "AUDIO_UPLOAD_MAX_SIZE", 1024)))
            resp = get_audio_upload_url(
                _req(
                    "POST",
                    f"/api/student/clap-assignments/{ASSIGN_ID}/audio-upload-url",
                    None,
                    {
                        "item_id": _item_for_audio,
                        "mime_type": _mime,
                        "extension": _ext,
                        "file_size": _size,
                    },
            ...
            record(
                "SP6",
                "POST /student/clap-assignments/{id}/audio-upload-url  (200 or 503)",
                resp,
                [200, 503],
            )
        else:
            skip("SP6", "POST audio upload URL", "No assignment item found")
    else:
        skip("SP6", "POST audio upload URL", "No assignment in DB")

    if ASSIGN_AUDIO_ITEM_ID and SID and ASSIGN_ID:
        resp = student_retrieve_audio(
            _req(
                "GET",
                f"/api/student/clap-items/{ASSIGN_AUDIO_ITEM_ID}/audio?assignment_id={ASSIGN_ID}",
                None,
                student_id=SID,
            ),
            ASSIGN_AUDIO_ITEM_ID,
        )
        ...
        resp = get_playback_status(
            _req(
                "GET",
                f"/api/student/clap-items/{ASSIGN_AUDIO_ITEM_ID}/playback-status?assignment_id={ASSIGN_ID}",
                None,
                student_id=SID,
            ),
            ASSIGN_AUDIO_ITEM_ID,
        ...
        skip("SP7", "GET student audio file", "No audio item or assignment in DB")
        skip("SP8", "GET student playback-status", "No audio item or assignment in DB")

    if ASSIGN_ITEM_ID and ASSIGN_ID and SID:
        resp = submit_response(
            _req(
                "POST",
                f"/api/student/clap-assignments/{ASSIGN_ID}/submit",
                None,
                {
                    "item_id": ASSIGN_ITEM_ID,
        ...
        resp = finish_component(
            _req(
                "POST",
                f"/api/student/clap-assignments/{ASSIGN_ID}/components/{ASSIGN_CID}/finish",
                None,
                {},
                student_id=SID,
            ),
        ...
        skip("SP9", "POST submit response", "No assignment item found")
        skip("SP10", "POST finish component", "No assignment item found")
    ...
    if _has_openai and AI_ATTEMPT_ID:
        ...
        record("AI1", "POST /api/evaluate/speaking  (multipart)", resp, [200, 400, 503]) # 400 if Whisper fails, 503 if OpenAI unavailable
        ...
        record("AI2", "POST /api/evaluate/writing", resp, [200, 503])
    else:
    ...
    if _created_test_id:
        try:
            Test.objects.filter(id=_created_test_id).delete()
            print(f"  [CLEANUP] Deleted test: {_created_test_id}")
        except: pass

    if _created_audio_item_id:
        try:
            from api.models import AdminAudioFile
            _admin_audio = AdminAudioFile.objects.filter(item_id=_created_audio_item_id).first()
            if _admin_audio:
                _admin_audio.delete_file()
                _admin_audio.delete()
                print(f"  [CLEANUP] Deleted admin audio for item: {_created_audio_item_id}")
        except: pass

    if _created_item_ids:
        try:
            ClapTestItem.objects.filter(id__in=_created_item_ids).delete()
            print(f"  [CLEANUP] Deleted test items: {len(_created_item_ids)}")
        except: pass
