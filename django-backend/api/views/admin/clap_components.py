...
def clap_component_detail_handler(request, component_id):
    ...
    PATCH: Update component details (e.g. max_marks)
    """
    # Auth check
    ...
    if request.method == "GET":
        ...
        try:
            ...
            # Note: duration_minutes is kept in sync because ComponentAttempt
            # deadline enforcement reads it from ClapSetComponent.
...
def force_sync_timers(request, test_id):
    ...
    Force-propagates all master component settings (duration_minutes, max_marks)
    to every matching ClapSetComponent in this test.
    ...
    for master in ClapTestComponent.objects.filter(clap_test=clap_test):
        ...
        logger.info(
            ...
            f"→ {count} set component(s)"
        )

    return JsonResponse({
        'success': True,
        'message': f'Synced settings to {total_synced} set component(s)',
        'details': details,
    })
